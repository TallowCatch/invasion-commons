...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a Harvest Commons PPO checkpoint.")
    ...
    parser.add_argument("--output-prefix", default="results/runs/rl_harvest/harvest_rl_eval")
    return parser.parse_args()


def _cfg_from_checkpoint(meta: dict) -> HarvestCommonsConfig:
    cfg_dict = dict(meta.get("harvest_config", {}))
    if not cfg_dict:
        raise ValueError("Checkpoint does not contain harvest_config metadata.")
...
def main() -> None:
    ...
    train_episode_df, train_summary = evaluate_self_play_policy(
        cfg=cfg,
        condition=str(checkpoint_meta.get("condition", "")),
        policy=policy,
        ...
        deterministic=True,
        prefix="train",
        government_params=checkpoint_meta.get("government_params"),
        device=device,
    )
