...
def main() -> None:
    ...
    if not runs_files and not gen_files and not strat_files:
    ...
    print(f"Merged {len(runs_files)} run shard(s), {len(gen_files)} generation shard(s), {len(strat_files)} strategy shard(s).")
